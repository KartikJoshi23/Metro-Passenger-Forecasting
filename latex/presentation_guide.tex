...
future-work list.}
\q{Why do the passenger numbers look realistic --- aren't tap samples tiny?}
\an{The raw open extract is a \emph{sample} (about 1/100 scale), so we anchor the displayed numbers to
\textbf{real RTA figures}: the busiest station (BurJuman) is set to its true volume and the rest follow
the \emph{official monthly per-station ridership} (also from Dubai Pulse). This self-checks against
public data --- our network totals about 826k entries/day, matching Dubai Metro's real
$\approx$807k/day (294.7M in 2025), and BurJuman is $\approx$6\% of the network, exactly its real
share. The model still provides the \emph{shape} and the next-hour forecast; only the units are made
realistic.}
\q{What are the limitations of your data?}
\an{The tap extract is a \emph{sample}, and a day's overall level varies; we handle this with
per-station scaling, calibration and the real-magnitude anchoring above. Shapes, peaks and rankings
are genuine; full-population AFC data would simply sharpen the exact counts.}
